...
#include <cmath>
using namespace std;
string round_keys[16]; // Array to hold 16 keys
string pt; // String to hold the plain text
...
unsigned long long convertBinaryToDecimal(string binary)
{
...
string shift_left_once(string key_chunk){ 
    string shifted="";  
        for(unsigned long long i = 1; i < 28; i++){ 
            shifted += key_chunk[i]; 
        } 
        shifted += key_chunk[0];   
    return shifted; 
...
string shift_left_twice(string key_chunk){ 
    ...
    for(unsigned long long i = 0; i < 2; i++){ 
        for(unsigned long long j = 1; j < 28; j++){ 
            shifted += key_chunk[j]; 
        } 
        shifted += key_chunk[0]; 
        key_chunk= shifted; 
...
void generate_keys(string key){
	// The PC1 table
	unsigned long long pc1[56] = {
		57,49,41,33,25,17,9,1,58,50,42,34,26,18,10,2,59,51,43,35,27,19,11,3,60,52,44,36,
		63,55,47,39,31,23,15, 7,62,54,46,38,30,22, 14,6,61,53,45,37,29, 21,13,5,28,20,12,4 
	};
	// The PC2 table
	unsigned long long pc2[48] = {
	...
	string perm_key =""; 
	for(unsigned long long i = 0; i < 56; i++){ 
		perm_key+= key[pc1[i]-1]; 
	} 

	// 2. Dividing the key unsigned long longo two equal halves
	...
	for(unsigned long long i=0; i<16; i++){ 
		...
		for(unsigned long long i = 0; i < 48; i++){ 
...
string DES(){ 
	...
	// 3. Encrypting the plain text 16 times  
	for(int i=0; i<16; i++) { 
		...
		string res = ""; 
		// The result is divided into 8 equal parts and passed through 8 substitution boxes. 
		// After passing through a substituion box, each box is reduces from 6 to 4 bits.
		for(int i=0;i<8; i++){ 
		...
		// The result is xored with the left half & then the left and the right parts of the plain text are swapped 
...
int main(){ 
	string key;
	key= "1001010101011101100001001000110000010011100110110110011001101111";
		
    cout<<"Key: "<<key<<" = "<<convertBinaryToDecimal(key)<<endl;  
	pt= "1101010111100110111100111010101011110011010001001100100101001101";
		//  1111111111111111111111111111111111001001001010111011110011110111
	// cout<<"Enter the key and plaunsigned long longext in binary: \nKey: ";
	// cin>>key;
	// cout<<"Plaunsigned long longext: ";
	// cin>>pt;
	string apt = pt;

  	generate_keys(key); 
    cout<<"Plain text: "<<pt<<" = "<<convertBinaryToDecimal(pt)<<endl; 
    string ct= DES(); 
    cout<<"Ciphertext: "<<ct<<" = "<<convertBinaryToDecimal(ct)<<endl;
	...
	cout<<"Decrypted text: "<<decrypted<<" = "<<convertBinaryToDecimal(decrypted)<<endl;
